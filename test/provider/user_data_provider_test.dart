...
//        UserDataProvider(fireStoreDb: fireStoreMock);
//
//    test(
//        'save detail from google return a user with detail from firebase object',
//        () async {
...
//      when(fireStoreMock.collection(any)).thenReturn(collectionReferenceMock);
//      when(collectionReferenceMock.document(any))
//          .thenReturn(documentReferenceMock);
//      expect(await documentReferenceMock.snapshots().isEmpty,
//          true); // no data  is save for this user
//      User user =
//          await userDataProvider.saveDetailsFromGoogleAuth(FirebaseUserMock());
//      expect(await documentReferenceMock.snapshots().isEmpty,
//          false); // data is save. snapshot has one snapshot
//      documentSnapshotMock = await documentReferenceMock.get();
//      expect(await documentSnapshotMock.data['uid'],
//          'uid'); // checking  if data form  the passed FirebaseUser object is return in user
//      expect(user.name, 'John Doe');
//      expect(user.photoUrl, 'http://www.adityag.me');
//    });
//
//    test(
//        'save detail from google if there is not existing image write the image from firebase user',
//        () async {
//      CollectionReferenceMock collectionReferenceMock =
//          CollectionReferenceMock();
//      DocumentReferenceMock documentReferenceMock = DocumentReferenceMock();
...
//          .thenReturn(documentReferenceMock);
//      expect(await documentReferenceMock.snapshots().isEmpty,
//          true); // no data is save ,fresh user
//      User user =
//          await userDataProvider.saveDetailsFromGoogleAuth(FirebaseUserMock());
//      expect(user.name, 'John Doe');
//      expect(user.photoUrl,
//          'http://www.adityag.me'); // image from firebase object is written
//    });
//    test(
//        'save detail from google Auth if  there is existing image, do not write the image from firebase',
//        () async {
//      CollectionReferenceMock collectionReferenceMock =
//          CollectionReferenceMock();
//      DocumentSnapshotMock documentSnapshotMock = DocumentSnapshotMock(
//          mockData:
//              Map<String, dynamic>.from({'photoUrl': 'http://www.google.com'}));
//      DocumentReferenceMock documentReferenceMock = DocumentReferenceMock();
//      documentReferenceMock =
//          DocumentReferenceMock(documentSnapshotMock: documentSnapshotMock);
//      when(fireStoreMock.collection(any)).thenReturn(collectionReferenceMock);
//      when(collectionReferenceMock.document(any))
//          .thenReturn(documentReferenceMock);
//      expect(await documentReferenceMock.snapshots().isEmpty,
//          false); //snapshot are not empty because existing user
//      User user =
//          await userDataProvider.saveDetailsFromGoogleAuth(FirebaseUserMock());
//      expect(await documentReferenceMock.snapshots().isEmpty, false);
//      expect(user.name, 'John Doe');
//      expect(user.photoUrl,
//          'http://www.google.com'); // for existing user the is not overwritten
//    });
//    test('save Profile detail save the detail', () async {
//          CollectionReferenceMock collectionReferenceMock = CollectionReferenceMock();
//    DocumentSnapshotMock documentSnapshotMock = DocumentSnapshotMock();
//    DocumentReferenceMock documentReferenceMock = DocumentReferenceMock();
//      documentReferenceMock = DocumentReferenceMock(); //create a user
//      when(fireStoreMock.collection(any)).thenReturn(collectionReferenceMock);
//      when(collectionReferenceMock.document(any))
//          .thenReturn(documentReferenceMock);
//      expect(await documentReferenceMock.snapshots().isEmpty, true);
//      User user = await userDataProvider.saveProfileDetail(
//          'uid', 'http://www.githup.com', 18, 'Nguyen');
//      expect(await documentReferenceMock.snapshots().isEmpty, false);
//      expect(user.age, 18); // checking if passed data is save
//      expect(user.name, 'John Doe');
//      expect(user.photoUrl, 'http://www.githup');
//    });
//
//    test('is  profile complete works properly', () async {
...
//      documentReferenceMock =
//          DocumentReferenceMock(documentSnapshotMock: documentSnapshotMock);
//      documentReferenceMock.setData({'username': 'John Doe', 'age': '18'});
//      when(fireStoreMock.collection(any)).thenReturn(collectionReferenceMock);
//      when(collectionReferenceMock.document(any))
//          .thenReturn(documentReferenceMock);
...
//
//      //clear profile data. Should return false now
...
//    SharedObjects.prefs = sharedPreferencesMock;
    ...
    test(
        'saveDetailsFromGoogleAuth returns a user with the details from FirebaseUser Object passed', () async {
      ...
      expect(await documentReference
          .snapshots()
          .isEmpty,
    ...
    test(
        'saveDetailsFromGoogleAuth if there is not existing image write the image from firebase user', () async {
      CollectionReferenceMock collectionReference = CollectionReferenceMock();
      DocumentReferenceMock documentReference = DocumentReferenceMock();
//
...
//      when(SharedObjects.prefs.setString(any, any)).thenAnswer((_)=>Future.value(true));
    ...
    test(
        'saveDetailsFromGoogleAuth if there is existing image, do not write the image from firebase user', () async {
      CollectionReferenceMock collectionReference = CollectionReferenceMock();
      DocumentSnapshotMock documentSnapshot = DocumentSnapshotMock(
          mockData: Map<String, dynamic>.from({
            'photoUrl': 'http://www.google.com'
          })); //create a snapshot first to mock existing user
      DocumentReferenceMock documentReference = DocumentReferenceMock();
      documentReference =
...
//      expect(user.photoUrl, 'http://www.github.com');
    ...
    test('isProfileComplete works properly', () async {
      ...
      documentReference.setData(
          {'username': 'johndoe', 'age': 18, 'photoUrl': 'http://adityag.me'});
      //when(sharedPreferencesMock.getString(any)).thenReturn('uid');
      when(fireStore.collection(any)).thenReturn(collectionReference);
      when(collectionReference.document(any)).thenReturn(documentReference);
      expect(await documentReference
